    function ccgSegsA2B2A = CCGofSegments(trial, pfPars, epochs, varargin)
    ...
    [IF_REPORT_FIG, pairs2analyse, IF_SAVE, trialName, fileTag] = DefaultArgs(varargin, {0, [1:size(pfPars.selectedPairs, 1)],1, 'crt1', []});
...
%         abDir = {'a2b', 'b2a'};
        for kDir = 1 : 2
            ...
            for kPair = 1 : nPairs 
                ...
                idx = ismember(pfPars.selectedPairs ,cellPairs(kPair,:), 'rows');
                if ~isempty(epochs{idx})
                    ...
                    % load Clu Res for times when trajectories were inside the PF
        ...
        if IF_SAVE
            save(['~/data/analysis/' filebase '/' mfilename '.' trial.trialName '.mat'], 'ccgSegsA2B2A');
        end
    end
