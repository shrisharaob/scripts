function out = SelectCells(trial, varargin)
    ...
    [LoadData, Arena, IF_PLOT, type] = DefaultArgs(varargin, {{'CluRes','Pfs'}, 'cof', 0, 'load'});
%     session = MTASession(filebase, LoadData, Arena);
    switch type
      case 'load'
        if FileExists([trial.paths.analysis, trial.filebase '.' mfilename '.mat'])
            load([trial.paths.analysis, trial.filebase '.' mfilename '.mat']);   
        end
      case 'compute'
        if FileExists([trial.paths.analysis, '/', filebase '.NeuronQuality.mat'])
            ...
            nq = NeuronQuality([trial.paths.data, trial.filebase], [],[],[],1);
        end
