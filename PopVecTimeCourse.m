function [popVec, refVector] = PopVecTimeCourse(filebase, varargin)
    ...
    [commonCluIdx, binSize, roi, tolerence, IF_OVERWRITE] = DefaultArgs(varargin, {[],  10,  {'CA3'}, 1e-1,1});
    %binSize in degrees
    % .1 ~= 5 degrees
    %    if FileExists(
    sK.roi = roi;
    ...
    res = gt.res(ismember(gt.clu, commonCluIdx)); % load res only for the units in roi
    clu = gt.clu(ismember(gt.clu, commonCluIdx));
    ...
    refVector = nansum(gt.pfObject.smoothRateMap(:, :, ismember(gt.pfObject.acceptedUnits, commonCluIdx)), 3);
    refVector = refVector(:);
    ...
    for kPopVec = 1 : length(thetaBoundaries) - 1
        ...
        for kClu = 1 : length(commonCluIdx)
            curRes = SelectPeriods(res(clu == commonCluIdx(kClu)), thetaPeriods(kPopVec, :),'d',1,1);
            if ~isempty(curRes)
