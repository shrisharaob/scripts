function out = GetGoodPFUnits(filebase, varargin)
    ...
    [arena, roi, IF_SmthRM, IF_REPORTFIG] = DefaultArgs(varargin, {{'bigSquare', 'linear'}, {'CA3'}, 1, 1});
    kenjiSearch.roi = roi;
    ...
    for kTr = 1 : nTrials
        ...
        roiPFPairs{kTr} = gta{kTr}.pfObject.selectedPairs(ismember(gta{kTr}.pfObject.selectedPairs, nchoosek(commonClus, 2), 'rows'), :);
    end
    ...
    fprintf(fp, ['\n # good PF units :' num2str(length(out.goodUnits))]);
